...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_student'])) {
        ...
        $sql = "UPDATE users SET student_id='$student_id', name='$name', email='$email' WHERE id=$id";

        if (!empty($_POST['password'])) {
            $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
            $sql = "UPDATE users SET student_id='$student_id', name='$name', email='$email', password='$password', change_password=1 WHERE id=$id";
        }

        if ($conn->query($sql) === TRUE) {
            echo "<script>window.location.href='?page=students&msg=updated';</script>";
        } else {
            $error_msg = "Error: " . $conn->error;
        }
    }
...
// Fetch Students with Total XP calculation
// Fetch Students with Total XP calculation
// Select specific columns to avoid GROUP BY strict mode issues
...
if (isset($_GET['delete_id'])) {
    ...
    if ($student_query && $student = $student_query->fetch_assoc()) {
        if (!empty($student['profile_photo'])) {
            $photo_path = "../uploads/profile_photos/" . $student['profile_photo'];
...
if (isset($_GET['toggle_id']) && isset($_GET['status'])) {
    ...
    echo "<script>window.location.href='?page=students&msg=status_updated';</script>";
}
...
<div class="flex flex-col gap-6">
    ...
    <?php if (isset($success_msg) || isset($_GET['msg'])): ?>
        <div class="p-4 border border-nexus-green/50 bg-nexus-green/10 text-nexus-green text-xs font-mono">
            > SUCCESS: <?php echo isset($success_msg) ? $success_msg : "Record updated successfully."; ?>
        </div>
    <?php endif; ?>
                        ...
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr class="hover:bg-white/5 transition-colors group">
                                ...
                                <td class="px-6 py-4 flex items-center gap-3">
                                    ...
                                    <!-- Toggle Status -->
                                    <a href="?page=students&toggle_id=<?php echo $row['id']; ?>&status=<?php echo $row['status']; ?>"
                                        ...
                                        <i
                                            class="fa-solid <?php echo $row['status'] == 'active' ? 'fa-toggle-on' : 'fa-toggle-off'; ?> text-lg"></i>
                                    </a>

                                    <!-- Delete -->
                                    <a href="?page=students&delete_id=<?php echo $row['id']; ?>"
...
<div id="studentModal"
    ...
    <div class="holo-card w-full max-w-md p-6 rounded-xl relative">
        ...
        <form method="POST" class="space-y-4" action="?page=students">
            <?php if ($edit_student): ?>
